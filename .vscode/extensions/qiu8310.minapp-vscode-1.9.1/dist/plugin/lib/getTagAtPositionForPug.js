...
function getTagAtPosition(doc, pos) {
    ...
    if (SINGLE_LINE_TAG_REGEXP.test(line)) {
        ...
        if (index < prefix.length) {
            ...
            return {
                name,
                attrs: getTagAtPosition_1.getAttrs(attrstr),
                posWord: name,
                isOnTagName: true,
                isOnAttrName: false,
                isOnAttrValue: false,
    ...
    while (lineNum >= 0) {
        ...
        if (text) {
            ...
            if (text.indexOf('(') > 0 && MULTIPLE_LINE_TAG_REGEXP.test(text)) {
                let name = RegExp.$2;
                Object.assign(attrs, getTagAtPosition_1.getAttrs((RegExp.$5 || '').trim()));
                return name;
            }
            let left = getTagAtPosition_1.getAttrs2(text, attrs);
            if (left)
                return false;
        }
        lineNum--;
...
//# sourceMappingURL=getTagAtPositionForPug.js.map